...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PSD_Params_t DPPParams)
{
    ...
    ret |= CAEN_DGTZ_SetDPPAcquisitionMode(handle, Params.AcqMode, CAEN_DGTZ_DPP_SAVE_PARAM_EnergyAndTime); // EnergyOnly
    ...
    ret |= CAEN_DGTZ_SetExtTriggerInputMode(handle, CAEN_DGTZ_TRGMODE_DISABLED); // EXTOUT_ONLY or ACQ_ONLY or ACQ_AND_EXTOUT
    ...
    for (i = 0; i < MaxNChannels; i++)
    {
        if (Params.ChannelMask & (1 << i))
        {
            ...
            ret |= CAEN_DGTZ_SetChannelDCOffset(handle, i, 0x3333); // 0x0000 to 0xFFFF (0 to 100%)
    ...
    uint32_t RData;
    ret |= CAEN_DGTZ_ReadRegister(handle, 0x8000, &RData);
    // std::cout << "RData : " << std::bitset<32>(RData) << std::endl;
    ret |= CAEN_DGTZ_WriteRegister(handle, 0x8000, (RData | (1 << 17)));
...
int main(int argc, char *argv[])
{
    ...
    // TStopwatch ClockTime;

    CAEN_DGTZ_ErrorCode ret;
    int ChannelMask = 0b00000111;
    ...
    for (b = 0; b < MAXNB; b++)
    {
        ...
        * Communication Parameters   *
        ...
        Params[b].LinkType = CAEN_DGTZ_USB_V4718; // Link Type  // CAEN_DGTZ_USB_A4818
        Params[b].VMEBaseAddress = 0x11110000;    // For direct USB connection, VMEBaseAddress must be 0
        Params[b].PID = A4818_PID;                // 16367 for A4818 and 17734 for V4718
        ...
        Params[b].IOlev = CAEN_DGTZ_IOLevel_NIM; // NIM or TTL
        /****************************\
        *  Acquisition parameters    *
        \****************************/
        Params[b].AcqMode = CAEN_DGTZ_DPP_ACQ_MODE_List; // Mixed          // or CAEN_DGTZ_DPP_ACQ_MODE_List
        Params[b].RecordLength = 140;                    // Num of samples of the waveforms (only for Oscilloscope mode)
        Params[b].ChannelMask = 0b00000111;              // 0x07; //0xFF                              // Channel enable mask
        // std::cout << "Init Channel Mask : " << Params[b].ChannelMask << std::endl;
        ...
        *      DPP parameters        *
        ...
        for (ch = 0; ch < MaxNChannels; ch++)
        {
            ...
            DPPParams[b].lgate[ch] = 150; // Long Gate Width (N*2ns for x730  and N*4ns for x725)
            DPPParams[b].sgate[ch] = 40;  // Short Gate Width (N*2ns for x730  and N*4ns for x725)
            DPPParams[b].pgate[ch] = 25;  // Pre Gate Width (N*2ns for x730  and N*4ns for x725)
            if (b == 2)
            {
                DPPParams[b].lgate[ch] = 1200; // Long Gate Width (N*2ns for x730  and N*4ns for x725)
                DPPParams[b].sgate[ch] = 40;   // Short Gate Width (N*2ns for x730  and N*4ns for x725)
                DPPParams[b].pgate[ch] = 25;
            }
            /* Self Trigger Mode:
            0 -> Disabled
            1 -> Enabled */
            ...
            DPPParams[b].trgc[ch] = CAEN_DGTZ_DPP_TriggerConfig_Threshold; // Threshold or peak //CAEN_DGTZ_DPP_TriggerMode_Normal;
            ...
            DPPParams[b].discr[ch] = CAEN_DGTZ_DPP_DISCR_MODE_LED; // CFD or LED

            /*CFD delay (N*2ns for x730  and N*4ns for x725)  */
            DPPParams[b].cfdd[ch] = 3;

            /*CFD fraction: 0->25%; 1->50%; 2->75%; 3->100% */
            DPPParams[b].cfdf[ch] = 2;
            ...
            DPPParams[b].csens[ch] = 2;
            if (b == 2)
                DPPParams[b].csens[ch] = 4;
        }
        /* Pile-Up rejection Mode
        CAEN_DGTZ_DPP_PSD_PUR_DetectOnly -> Only Detect Pile-Up
        CAEN_DGTZ_DPP_PSD_PUR_Enabled -> Reject Pile-Up */
        DPPParams[b].purh = CAEN_DGTZ_DPP_PSD_PUR_Enabled; // Reject pileup
        DPPParams[b].purgap = 1000;                        // Purity Gap in LSB units (1LSB = 0.12 mV for 2Vpp Input Range, 1LSB = 0.03 mV for 0.5 Vpp Input Range )
    ...
    for (b = 0; b < MAXNB; b++)
    {
        ...
        ret = CAEN_DGTZ_OpenDigitizer2(Params[b].LinkType, &(Params[b].PID), 0, Params[b].VMEBaseAddress, &handle[b]);
        ...
        if (ret)
        {
            ...
            std::cout << "Error code number : " << ret << std::endl;
            exit(1);
        }
        ...
        if (ret)
        {
            std::cout << "Can't read board info" << std::endl;
            exit(2);
        }
        std::cout << "\nConnected to CAEN Digitizer Model " << BoardInfo.ModelName << " recognized as board " << b << std::endl;
        std::cout << "ROC FPGA Release : " << BoardInfo.ROC_FirmwareRel << std::endl;
        std::cout << "AMC FPGA Release : " << BoardInfo.AMC_FirmwareRel << std::endl;
        std::cout << std::endl;
    ...
    for (b = 0; b < MAXNB; b++)
    {
        ret = (CAEN_DGTZ_ErrorCode)ProgramDigitizer(handle[b], Params[b], DPPParams[b]);
        if (ret)
        {
            std::cout << "Failed to program the digitizer" << std::endl;
            exit(4);
        }

        uint32_t RData2;
        ret = CAEN_DGTZ_ReadRegister(handle[b], 0x8084, &RData2);
        std::cout << "RData2 : " << std::bitset<32>(RData2) << std::endl;
        ret = CAEN_DGTZ_WriteRegister(handle[b], 0x8084, (RData2 | (2 << 8)));
    }
    ...
    ret = CAEN_DGTZ_MallocDPPWaveforms(handle[0], (void **)&Waveform, &AllocatedSize);
    if (ret)
    {
    ...
    int TimeLimit;
    std::cout << "How long you want to measure (in sec) ? : ";
    std::cin >> TimeLimit;

    char outfname[200];
    std::cout << "filename? : ";
    std::cin >> outfname;
    // sprintf(outfname, outfname);
    TFile *fout = new TFile(outfname, "recreate");
    TTree *tr = new TTree("DATA", "CAEN VME LaBr3 DATA");
    // short Board = 0;
    ...
    tr->Branch("Ch", &Channel, "Ch/s");
    tr->Branch("TS", &TS, "TS/l");
    tr->Branch("Energy", &Energy, "Energy/s");
    tr->Branch("E_Short", &E_Short, "E_Short/s");
    ...
    // ClockTime.Reset();
    int T_start = 0, T_end = 0, T_elapsed = 0;
    for (b = 0; b < MAXNB; b++)
    {
        // Start Acquisition
    ...
    AcqRun = 1;
    T_start = time(NULL);
    while (!Quit && T_elapsed < TimeLimit)
    {
        ...
        // std::cout << "Channel Mask after digitizer start : " << ChannelMask << std::endl;
        ...
        for (b = 0; b < MAXNB; b++)
        {
            ...
            for (ch = 0; ch < MaxNChannels; ch++)
            {
                // std::cout << "NumEvents of Channel " << ch << " : " <<NumEvents[ch] << std::endl;
                // std::cout << "Params.ChannelMask " << " : " << (Params[b].ChannelMask) << std::endl;
                // std::cout << "Masking Channel " << ch << " : " << (Params[b].ChannelMask & (1 << ch)) << std::endl;

                // if (!(Params[b].ChannelMask & (1 << ch)))
                if (!(ChannelMask & (1 << ch)))
                {
                    // std::cout << "Masked ch : " << ch << std::endl;
                    continue;
                }
                ...
                for (ev = 0; ev < NumEvents[ch]; ev++)
                {
                    ...
                    if ((Events[ch][ev].ChargeLong > 0) && (Events[ch][ev].ChargeShort > 0))
                    {
                        // Fill the tree
                        Channel = ch;
                        ULong64_t Extras_shift = (Events[ch][ev].Extras >> 16);
                        // std::cout << "Extras : " << Events[ch][ev].Extras << std::endl;
                        // std::cout << "Extras_TS : " << (Events[ch][ev].Extras >> 16) << std::endl;
                        // std::cout << "Extras_TS_shift : " << (Extras_shift << 32) << std::endl;
                        TS = (Extras_shift << 31) + (ULong64_t)Events[ch][ev].TimeTag;
                        // std::cout << "TS : " << TS << std::endl;
                        Energy = Events[ch][ev].ChargeLong;
                        E_Short = Events[ch][ev].ChargeShort;
                        tr->Fill();

                        // Fill the histograms
                        // std::cout << "Charge long : " << Events[ch][ev].ChargeLong << std::endl;
                        // std::cout << "Charge short : " << Events[ch][ev].ChargeShort << std::endl;
                    ...
                    if ((Params[b].AcqMode != CAEN_DGTZ_DPP_ACQ_MODE_List) && DoSaveWave[b][ch] && (ev == 0))
                    {
                        ...
                        WaveLine = (int16_t *)Waveform->Trace1; // First trace (for DPP-PSD it is ALWAYS the Input Signal)
                        SaveWaveform(b, ch, 1, size, WaveLine);

                        WaveLine = (int16_t *)Waveform->Trace2; // Second Trace (if single trace mode, it is a sequence of zeroes)
